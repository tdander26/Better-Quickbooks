// Seeds the chart of accounts, default auto-categorization rules, and a set of
// realistic mock Chase/Ally transactions so every report and chart has data to
// render before a real SimpleFIN feed is connected.
//
// Idempotent-ish: it clears transactional/mock data and re-seeds. Safe to re-run
// via `npm run db:seed` (or `npm run db:reset` to wipe the whole DB first).

import { PrismaClient } from "@prisma/client";

const prisma = new PrismaClient();

// Local cents helper (seed avoids "@/..." path aliases so tsx runs cleanly).
...
// Default rules. priority: lower runs first. Transfers first so internal money
// movement never gets miscategorized as income/expense.
const RULES: {
  name: string;
  priority: number;
  matchField: string;
...
async function main() {
  console.log("Seeding Better-QuickBooks...");
  ...
  const chase = await prisma.account.create({
    data: { name: "Total Checking", institution: "Chase", type: "bank", classification: "asset", openingBalanceCents: c(5000), openingDate: new Date("2026-04-01"), reportedBalanceCents: c(8420.55), balanceDate: new Date("2026-07-02"), sortOrder: 0 },
  });
  const allyChecking = await prisma.account.create({
    data: { name: "Interest Checking", institution: "Ally", type: "bank", classification: "asset", openingBalanceCents: c(3000), openingDate: new Date("2026-04-01"), reportedBalanceCents: c(4115.2), balanceDate: new Date("2026-07-02"), sortOrder: 1 },
  });
  const allySavings = await prisma.account.create({
    data: { name: "Online Savings", institution: "Ally", type: "bank", classification: "asset", openingBalanceCents: c(20000), openingDate: new Date("2026-04-01"), reportedBalanceCents: c(20180.4), balanceDate: new Date("2026-07-02"), sortOrder: 2 },
  });
  const sapphire = await prisma.account.create({
    data: { name: "Sapphire Card", institution: "Chase", type: "credit_card", classification: "liability", openingBalanceCents: c(0), openingDate: new Date("2026-04-01"), reportedBalanceCents: c(-1840.32), balanceDate: new Date("2026-07-02"), sortOrder: 3 },
  });

  // Transaction builder. amount is signed dollars: +inflow / -outflow.
  const rows: {
    acct: string;
    date: string;
    amount: number;
  ...
  const acctMap: Record<string, string> = {
    chase: chase.id,
    allyC: allyChecking.id,
    allyS: allySavings.id,
    card: sapphire.id,
  };

  // Recurring monthly items across Apr, May, Jun (+ early Jul).
  for (const [mo, label] of [["04", "Apr"], ["05", "May"], ["06", "Jun"]] as const) {
    // Patient revenue deposits (Stripe/Square + insurance)
    ...
    // Credit card payment (transfer from Chase to card)
  ...
  // Early July + a couple deliberately-uncategorized items to demo the workflow.
  ...
  rows.push({ acct: "card", date: "2026-07-02", amount: -47.5, payee: "Kwik Trip", description: "KWIK TRIP 442", pending: true }); // uncategorized
  rows.push({ acct: "card", date: "2026-07-03", amount: -96.13, payee: "Menards", description: "MENARDS PURCHASE", pending: true }); // uncategorized
  ...
  console.log(`  4 accounts, ${count} transactions`);
  console.log("Done.");
}
